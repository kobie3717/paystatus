...
export async function runSandboxLoop(): Promise<void> {
  console.log(`[${new Date().toISOString()}] Starting sandbox probe tick...`);
  for (const probe of probes) {
    await runProbe(probe);
  }
  console.log(`[${new Date().toISOString()}] Sandbox probe tick complete`);
}
...
async function writeCheck(
  ...
  await getPool().query(
    'INSERT INTO status_checks (provider, layer, status, latency_ms, error_code, error_message, drift_detected, drift_summary, confidence) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)',
    [provider, 'sandbox', status, latencyMs ?? null, errorCode ?? null, errorMessage ?? null, driftDetected, driftSummary ?? null, confidence]
  );
  console.log(`  ${provider} [sandbox]: ${status}${latencyMs ? ` (${latencyMs}ms)` : ''} [${confidence}]`);
}

async function maybeOpenIncident(provider: string, severity: string, summary: string): Promise<void> {
  const { rows } = await getPool().query(
    'SELECT id FROM incidents WHERE provider = $1 AND severity = $2 AND layer = $3 AND resolved_at IS NULL LIMIT 1',
    [provider, severity, 'sandbox']
  );
  if (rows.length === 0) {
    await getPool().query(
      'INSERT INTO incidents (provider, layer, started_at, severity, summary) VALUES ($1, $2, now(), $3, $4)',
      [provider, 'sandbox', severity, summary.slice(0, 500)]
    );
...
async function maybeCloseIncident(provider: string): Promise<void> {
  const { rowCount } = await getPool().query(
    'UPDATE incidents SET resolved_at = now() WHERE provider = $1 AND layer = $2 AND resolved_at IS NULL',
    [provider, 'sandbox']
  );
